...
#'@returns list of glm estimated beta, alpha coefficients with their p-values and adjusted q-values
#'@export
get_glm_coefs = function(data) {
  ...
  suppressWarnings({
    for (j in 1:ncol(A)) {
      Aj = A[, j]
      model = glm(cbind(Aj, nn-Aj)  ~ 0 + X + V,
                  family = binomial())
      alpha_glm = coef(model)
      glm_coef[j,] =  alpha_glm
      se[j] = summary(model)$coefficients[b_ind, 2]
      p_value[j] = coef(summary(model))[, 4][b_ind]
    }
  })
